...
for i in range(aktualni_rok, aktualni_rok + 5):
    velikonocni_nedele = datetime.date(i, 3, 31) - relativedelta(days=datetime.date(i, 3, 31).weekday() + 1)
    print(f"Velikonoční neděle v roce {i}: {velikonocni_nedele.strftime('%d. %m. %Y')}")

aktualni_rok = datetime.date.today().year
for rok in range(aktualni_rok, aktualni_rok + 10):  # Kontrolujeme následujících 10 let
    datum_stedreho_dne = datetime.date(rok, 12, 24)
    if datum_stedreho_dne.weekday() == 6:  # 6 odpovídá neděli (0 = pondělí, 1 = úterý, atd.)
        print(f"Štědrý den v neděli bude v roce {rok}.")
        break
